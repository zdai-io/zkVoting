...
(async () => {
  ...
  async function processVoting(_account, _anonymousAccount) {
    ...
    await voting.methods.deposit(pi_a, pi_b, pi_c, pubinputs).send({from:_account});
    ...
    await voting.methods.withdrawal(pi_a, pi_b, pi_c, pubinputs, to).send({from:_anonymousAccount});
