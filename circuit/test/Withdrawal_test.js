...
const main = async () => {
  try {
   ...
   const vk_verifier = fload("circuit/compiled/Withdrawal_verification_key.json");
    ...
    let hashes = Array.from(Array(9), (v,k) => rbigint(32) % alt_bn_128_q)
    const hash0 = serializeAndHashUTXO(withdrawal);
    hashes.push(hash0);
    hashes = shuffle(hashes);

    in_selector = Array(10);
    for(let i = 0; i < 10; i++){
      in_selector[i] = hash0 == hashes[i] ? 1 : 0;
    }
